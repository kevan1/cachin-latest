...
import { fetchNativeEvmBalance } from './evmBalanceService';
...
export async function fetchMultiChainBalances(
  ...
  const solanaBalances = solanaAddress
    ...
    : null;
  const avalancheNativeBalance = avalancheAddress
    ? await fetchNativeEvmBalance(ChainType.AVALANCHE, avalancheAddress, forceFresh)
    : null;
  ...
  if (avalancheNativeBalance !== null) {
    avalancheChainBalance = {
      chain: ChainType.AVALANCHE,
      nativeBalance: avalancheNativeBalance,
      nativeSymbol: 'AVAX',
      usdcBalance: 0,
      usdtBalance: 0,
      totalUsd: avalancheNativeBalance * prices.avax,
    };
  }
...
export async function fetchChainBalance(
  ...
  if (chainType === ChainType.AVALANCHE) {
    const nativeBalance = await fetchNativeEvmBalance(chainType, address, forceFresh);
    return {
      chain: ChainType.AVALANCHE,
      nativeBalance,
      nativeSymbol: 'AVAX',
      usdcBalance: 0,
      usdtBalance: 0,
      totalUsd: nativeBalance * prices.avax,
    };
  }
